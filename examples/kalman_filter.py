...
class LocalizationLinear:
    ...
    def propagate(self, state, sensor_input):
        """Standard piece-wise constant acceleration and velocity model. """
        dt, acc = sensor_input
        pos, speed = state
        pos = pos + dt * speed
        speed = speed + dt * acc
...
def main():
    ...
    def create_data(kalman, true_init, true_inputs, obs_freq):
        ...
        inputs = [gs.concatenate(
            (incr[:1], np.random.multivariate_normal(
                incr[1:], kalman.process_noise))) for
                incr in true_inputs]
    ...
    P0 = np.diag(P0)
    Q = 0.001 * gs.eye(model.dim_noise)
    N = 0.01 * gs.eye(model.dim_obs)
    initial_covs = (P0, Q, N)
    kalman.initialise_covariances(*initial_covs)

    true_state = gs.array([0, 0, 0])
    true_inputs = [gs.array([dt, 1., 0., 0.1]) for _ in range(n_traj)]
