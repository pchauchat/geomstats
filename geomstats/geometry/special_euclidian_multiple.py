"""The multiple special euclidean group SE_k(n).

i.e. the Lie group of rigid transformations in n dimensions with additional
vectors.
"""
...
@geomstats.vectorization.decorator(['vector', 'else', 'else'])
def _translation_vec_to_block_static(vec, n, k):
    """Turn the stacked vector of translations into a matrix.

    The rows of the returned matrix are the translations.

    Parameters
    ----------
    vec : array-like, shape=[n_vecs, n * k]
        The part of a point in vector form encoding the translations.

    Returns
    -------
    block : array-like, shape=[n_vecs, n, k]
    """
    n_vecs, _ = vec.shape
    block = gs.reshape(vec, (n_vecs, k, n))
    block = gs.transpose(block, (0, 2, 1))
    return block


@geomstats.vectorization.decorator(['matrix', 'else', 'else'])
def _translation_block_to_vec_static(block, n, k):
    """Turn a matrix of vectors into the stacked vector of translations.

    The translations are given by the rows of the matrix.

    Parameters
    ----------
    block : array-like, shape=[n_vecs, n, k]
        A matrix encoding translations into its rows.

    Returns
    -------
    block : array-like, shape=[n_vecs, k * n]
    """
    n_vecs, _, _ = block.shape
    block_transpose = gs.transpose(block, (0, 2, 1))
    return gs.reshape(block_transpose, (n_vecs, k * n))

class _MultipleSpecialEuclideanMatrices(GeneralLinear, LieGroup):
    ...
    def __init__(self, n, k):
        super(_MultipleSpecialEuclideanMatrices, self).__init__(
            dim=int((n * (n - 1)) / 2 + (k - 1) * n),
            default_point_type='matrix', n=n + k)
        self.rotations = SpecialOrthogonal(n=n)
        self.translations = Euclidean(dim=n * k)
        self.n = n
        self.k = k

    def get_identity(self):
        """Return the identity matrix."""
        return gs.eye(self.n + self.k, self.n + self.k)
    ...
    def belongs(self, point):
        ...
        """
        n_vecs = point_dim1, point_dim2 = point.shape
        belongs = (point_dim1 == point_dim2 == self.n + self.k)
        ...
        last_lines_first_block = point[..., self.n:, :self.n]
        last_lines_second_block = point[..., self.n:, self.n:]

        first_block_is_zeros = gs.allclose(
            last_lines_first_block, gs.zeros_like(last_lines_first_block))
        second_block_is_eye = gs.allclose(
            last_lines_second_block, gs.array([gs.eye(self.k)] * n_vecs))

        belongs = gs.logical_and(belongs, first_block_is_zeros)
        belongs = gs.logical_and(belongs, second_block_is_eye)
    ...
    def _is_in_lie_algebra(self, tangent_vec, atol=TOLERANCE):
        ...
        last_lines = tangent_vec[..., self.n:, :]
        all_zeros = ~ gs.any(last_lines, axis=-1)
    ...
    def _to_lie_algebra(self, tangent_vec):
        ...
        translation_mask = gs.concatenate(
            [translation_mask, gs.zeros((self.k, self.n + self.k))], axis=0)
    ...
    def random_uniform(self, n_samples=1, tol=1e-6):
        ...
        random_translation = _translation_vec_to_block_static(
            random_translation, self.n, self.k)
        ...
        last_lines_first_block = gs.zeros((n_samples, self.k, self.n + self.k))
        last_lines_second_block = gs.array([gs.eye(self.k)] * n_samples)
        last_lines = gs.concatenate(
            (last_lines_first_block, last_lines_second_block), axis=2)
        random_point = gs.concatenate(
            (random_point, last_lines), axis=1)
        if gs.shape(random_point)[0] == 1:
            random_point = gs.squeeze(random_point, axis=0)
        return random_point


class _MultipleSpecialEuclideanVectors(LieGroup):
    ...
    def __init__(self, n, k=1, epsilon=0.):
        dim = n * (n + 1) // 2 + (k - 1) * n
        LieGroup.__init__(
    ...
    def _translation_vec_to_block(self, vec):
        """Turn the stacked vector of translations into a matrix.
        ...
        block : array-like, shape=[n_vecs, n, k]
        """
        return _translation_vec_to_block_static(vec, self.n, self.k)

    @geomstats.vectorization.decorator(['else', 'matrix'])
    def _translation_block_to_vec(self, block):
        """Turn a matrix of vectors into the stacked vector of translations.
        ...
        Returns
        -------
        vec : array-like, shape=[n_vecs, k * n]
        """
        return _translation_block_to_vec_static(block, self.n, self.k)
    ...
    def get_identity(self, point_type=None):
        ...
        Returns
        -------
        identity : array-like, shape={[dim], [n + k, n + k]}
        """
    ...
    def matrix_from_vector(self, vec):
        ...
        trans_vec = self._translation_vec_to_block(trans_vec)
        mat = gs.concatenate((rot_mat, trans_vec), axis=2)
        last_lines_first_block = gs.zeros((n_vecs, 1, self.n))
        last_lines_second_block = gs.array([gs.eye(self.k)] * n_vecs)
        last_lines = gs.concatenate(
            (last_lines_first_block, last_lines_second_block), axis=2)
    ...
    def compose(self, point_a, point_b):
        ...
        translations_vec_a = point_a[..., dim_rotations:]
        translation_block_a = self._translation_vec_to_block(
            translations_vec_a)
        translations_vec_b = point_b[..., dim_rotations:]
        translation_block_b = self._translation_vec_to_block(
            translations_vec_b)
        ...
        composition_translation_block = gs.einsum(
            '...jk,...kl->...jl',
            rot_mat_a,
            translation_block_b) + translation_block_a
        composition_translation = self._translation_block_to_vec(
            composition_translation_block)
    ...
    def inverse(self, point):
        ...
        translations_vec = point[:, dim_rotations:]
        translations_block = self._translation_vec_to_block(translations_vec)
        ...
        inverse_translation_block = gs.einsum(
            '...jk,...kl->...jl',
            gs.transpose(inv_rot_mat, axes=(0, 2, 1)),
            -translations_block)
        inverse_translation = self._translation_block_to_vec(
            inverse_translation_block)
    ...
    def exp_from_identity(self, tangent_vec):
        ...
        translation_vec = tangent_vec[..., dim_rotations:]
        translation_block = self._translation_vec_to_block(translation_vec)
        exp_translation_block = gs.einsum(
            'ijk, ikl -> ijl', transform, translation_block)
        exp_translation = self._translation_block_to_vec(exp_translation_block)
    ...
    def log_from_identity(self, point):
        ...
        translation_vec = point[:, dim_rotations:]
        translation_block = self._translation_vec_to_block(translation_vec)

        log_translation_block = gs.einsum(
            'ijk, ikl -> ijl', transform, translation_block)
        log_translation = self._translation_block_to_vec(log_translation_block)
    ...
    def random_uniform(self, n_samples=1):
        ...
        random_point : array-like, shape=[..., dimension]
            Sample.
        """
        random_translations = self.translations.random_uniform(n_samples)
        random_rot_vec = self.rotations.random_uniform(n_samples)
        return gs.concatenate([random_rot_vec, random_translations], axis=-1)


class _MultipleSpecialEuclidean2Vectors(_MultipleSpecialEuclideanVectors):
    ...
    def __init__(self, k=1, epsilon=0.):
        super(_MultipleSpecialEuclidean2Vectors, self).__init__(
            n=2, k=k, epsilon=epsilon)
...
class _MultipleSpecialEuclidean3Vectors(_MultipleSpecialEuclideanVectors):
    ...
    def __init__(self, k=1, epsilon=0.):
        super(_MultipleSpecialEuclidean3Vectors, self).__init__(
            n=3, k=k, epsilon=epsilon)
...
class MultipleSpecialEuclidean(
    _MultipleSpecialEuclidean2Vectors, _MultipleSpecialEuclidean3Vectors,
        _MultipleSpecialEuclideanMatrices):
    ...
    def __new__(cls, n, k, point_type='matrix', epsilon=0.):
        ...
        if n == 2 and point_type == 'vector':
            return _MultipleSpecialEuclidean2Vectors(k, epsilon)
        if n == 3 and point_type == 'vector':
            return _MultipleSpecialEuclidean3Vectors(k, epsilon)
        if point_type == 'vector':
            raise NotImplementedError(
                'SE(n) is only implemented in matrix representation'
                ' when n > 3.')
        return _MultipleSpecialEuclideanMatrices(n, k)
